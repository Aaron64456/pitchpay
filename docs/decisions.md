...
| 24 Sep 2026 | SQL migrations committed to the repo and applied to the hosted Supabase project with `supabase db push` | Local Supabase stack via Docker; clicking through the Supabase SQL editor | Keeps the schema versioned and reproducible without a Docker install on Windows. A local stack can be added later if tests need one. |
| 24 Sep 2026 | RLS enabled on every table with **no** policies; all access goes through server-side code using the service role key | Permissive read policies for the anon key | The anon key is visible in the browser, so any policy is effectively public. Denying everything means game bank details can only be read through code we control. Revisit in Phase 6 if Realtime is used, since it obeys RLS. |
